...
class HudWorld extends World with HasUpdateMixin, HasGameRef<GameScene> {
  ...
  Future<void> _showPauseButton() async {
    SpriteComponent defaultSkin = SpriteComponent(
      sprite: Sprite(gameRef.images.fromCache(pathPauseButton)),
    );

    SpriteComponent selectedSkin = SpriteComponent(
      sprite: Sprite(gameRef.images.fromCache(pathPlayButton)),
    );
